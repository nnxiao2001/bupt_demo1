...
const compData0: ResourceDataType[] = [];
...
const App: React.FC = () => {
  ...
  const [compData, setCompData] = React.useState<ResourceDataType[]>(compData0);
  const [taskListData, setTaskListData] = React.useState<WorkDataType[]>(workData);

  const lightOn = () => {
    setOn(true);

    // Update the state to change the text in the last column for the first three rows of "任务列表"
    ...
    // Update the state to change the text in the last column for the first three rows of "计算资源表"
    setCompData((prevData) =>
      prevData.map((item, index) => {
        if (index < 3) {
          return { ...item, queue: '1-234321-2' };
        }
        return item;
      })
    );
  ...
  const lightOff = () => {
    ...
    // Restore the text in the last column for the first three rows of "任务列表" to '已处理'
    setTaskListData((prevData) =>
      prevData.map((item, index) => {
        if (index < 3) {
    ...
    // Restore the text in the last column for the first three rows of "计算资源表" to ' '
    setCompData((prevData) =>
      prevData.map((item, index) => {
        if (index < 3) {
          return { ...item, queue: ' ' };
  ...
  const customStateColumn = {
    ...
    width: 100,
    render: (text: string) => {
      // Customize the rendering based on the state
      if (on) {
        // If "开始调度" is active, apply red color
  ...
  const workColumnsWithCustomState = [...workColumns.slice(0, -1), customStateColumn];
